...
def parse_args() -> dotdict:
    ...
    parser.add_argument("--use_wandb", type=lambda x: bool(json.loads(x.lower())), default=True)
    ...
    parser.add_argument("--activation_width", type=int, default=768)
    ...
    parser.add_argument("--correlated_components", type=lambda x: bool(json.loads(x.lower())), default=True)
    ...
    parser.add_argument("--run_toy", type=lambda x: bool(json.loads(x.lower())), default=False)
    ...
    parser.add_argument("--use_residual", type=lambda x: bool(json.loads(x.lower())), default=False)  # whether to train on residual stream data
    ...
    parser.add_argument("--save_after_mini", type=lambda x: bool(json.loads(x.lower())), default=False)  # Whether to save the model after each mini run
    parser.add_argument("--upload_to_aws", type=lambda x: bool(json.loads(x.lower())), default=False)  # Whether to upload the model to aws after each mini run

    parser.add_argument("--refresh_data", type=lambda x: bool(json.loads(x.lower())), default=False)  # Whether to remake the dataset after each mini run
    parser.add_argument("--max_lines", type=int, default=100000)  # How many lines to read from the dataset
    # interpret
    parser.add_argument("--load_activation_dataset", type=lambda x: bool(json.loads(x.lower())), default=True) # path to dataset to load
    ...
    parser.add_argument("--tied_ae", type=lambda x: bool(json.loads(x.lower())), default=False) # whether to load pickle as a tied autoencoder
    ...
    parser.add_argument("--use_decoder", type=lambda x: bool(json.loads(x.lower())), default=True) # whether to use the transposed decoder instead of encoder in a non-tied ae
